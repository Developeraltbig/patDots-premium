import React, { useEffect, useState } from "react";
import { CheckCircle2 } from "lucide-react";
import "../styles/home/GenerateLoader.css";
const loadingSteps = [
  "Analyzing Invention Context",
  "Structuring Patent Sections",
  "Drafting Technical Claims",
  "Formatting to USPTO Standards",
  "Finalizing Variants",
];

const GenerateLoader = ({ draftType }) => {
  const [progress, setProgress] = useState(0);
  const [stepIndex, setStepIndex] = useState(0);

  useEffect(() => {
    // Prevent background scrolling while loading
    document.body.style.overflow = "hidden";

    // Progress bar stalls at 95% waiting for the backend Socket event
    const progressInterval = setInterval(() => {
      setProgress((prev) => {
        if (prev >= 95) return 95;
        if (prev < 40) return prev + 2;
        if (prev < 80) return prev + 0.8;
        return prev + 0.2;
      });
    }, 200);

    const stepInterval = setInterval(() => {
      setStepIndex((prev) =>
        prev < loadingSteps.length - 1 ? prev + 1 : prev,
      );
    }, 3500);

    return () => {
      document.body.style.overflow = "auto";
      clearInterval(progressInterval);
      clearInterval(stepInterval);
    };
  }, []);

  return (
    <div className="loader-page-overlay">
      <div className="loader-content">
        <div className="loader-logo-pulse">
          <svg
            viewBox="0 0 100 100"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="pulse-svg"
          >
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M25 80V20C25 20 45 20 60 20C79.33 20 95 35.67 95 55C95 74.33 79.33 90 60 90C50 90 40.83 85.83 34.33 79.17L44.83 68.67C48.67 72.5 54 75 60 75C71.05 75 80 66.05 80 55C80 43.95 71.05 35 60 35H40V80H25ZM60 65C65.52 65 70 60.52 70 55C70 49.48 65.52 45 60 45C54.48 45 50 49.48 50 55C50 60.52 54.48 65 60 65Z"
              fill="white"
            />
          </svg>
        </div>
        <h1 className="loader-title">Building Your Preview</h1>
        <p className="loader-subtitle">
          {draftType || "Provisional"} Application . 3 Variants Generating In
          Parallel
        </p>
        <div className="progress-bar-container">
          <div
            className="progress-bar-fill"
            style={{ width: `${progress}%` }}
          ></div>
        </div>
        <div className="loader-status">
          <CheckCircle2 size={16} className="status-icon" />
          <span>{loadingSteps[stepIndex]}</span>
        </div>
      </div>
    </div>
  );
};

export default GenerateLoader;
